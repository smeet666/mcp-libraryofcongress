/**
 * A small in-memory store, keyed by what produced the value.
 *
 * It exists to keep a conversation that asks the same thing twice from asking
 * the site twice, which matters more here than usual: the spacing between
 * requests is six seconds, so a repeated question costs a caller real waiting.
