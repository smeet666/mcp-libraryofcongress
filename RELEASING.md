...
| Repository           | `mcp-libraryofcongress` |
...
1. Bump the version in **five** places, which must stay in step:
   ...
   - `src/version.ts`, which feeds the User-Agent sent to the Library
...
```bash
npm view mcp-libraryofcongress version
npx -y mcp-libraryofcongress          # should start and wait on stdin
```
